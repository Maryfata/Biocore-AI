...
def render_academy():
    st.set_page_config(page_title="Academia Clínica Inteligente", layout="wide")
    
    tabs = st.tabs(["🎯 Misiones", "👥 Pacientes Virtuales", "🧬 Gemelo Digital", "📖 Tutor IA", "📊 Progreso"])
    
    with tabs[0]:
        st.header("🎯 Misiones Clínicas")
        st.write("Completa misiones de dificultad progresiva para aprender fisiología clínica.")
        
        selected_mission = st.selectbox("Selecciona una misión:", [m['title'] for m in CLINICAL_MISSIONS])
        mission = next(m for m in CLINICAL_MISSIONS if m['title'] == selected_mission)
        
        render_mission_card(mission)
        
        if st.button("▶️ Iniciar Misión"):
            st.session_state.current_mission = mission
            st.session_state.mission_started = True
            st.rerun()
        
        if st.session_state.get('mission_started'):
            st.markdown("---")
            patient_id = mission['patient']
            patient_data = VIRTUAL_PATIENTS[patient_id]
            
            st.markdown("### Información del Paciente")
            render_virtual_patient_card(patient_id, patient_data)
            
            create_digital_twin_for_patient(patient_data)
            
            st.markdown("---")
            st.markdown("### Responder Misión")
            findings = st.text_area("¿Cuáles son tus hallazgos e interpretación?")
            recommendation = st.text_area("¿Cuál es tu recomendación clínica?")
            
            if st.button("✅ Enviar Respuesta"):
                st.success("🎉 Misión completada!")
                st.balloons()
                st.markdown(f"**Puntos ganados:** +{mission['xp']} XP")
    
    with tabs[1]:
        st.header("👥 Pacientes Virtuales")
        st.write("Interactúa con pacientes virtuales de diferentes condiciones clínicas.")
        
        patient_names = list(VIRTUAL_PATIENTS.keys())
        selected_patient_key = st.selectbox("Selecciona un paciente:", patient_names)
        patient_data = VIRTUAL_PATIENTS[selected_patient_key]
        
        render_virtual_patient_card(selected_patient_key, patient_data)
        
        st.markdown("---")
        if st.button("📱 Interactuar con paciente"):
            st.info("Puedes hacer preguntas al paciente o evaluar sus síntomas usando el Tutor IA.")
    
    with tabs[2]:
        st.header("🧬 Gemelo Digital Interactivo")
        st.write("Selecciona un paciente y manípula su gemelo digital en tiempo real.")
        
        patient_key = st.selectbox("Paciente para gemelo:", list(VIRTUAL_PATIENTS.keys()), key='twin_select')
        patient = VIRTUAL_PATIENTS[patient_key]
        
        create_digital_twin_for_patient(patient)
    ...
    with tabs[3]:
        ...
        if TUTOR_AVAILABLE:
            ...
            st.warning("El tutor IA no está disponible. Verifica la instalación de dependencias.")
    ...
    with tabs[4]:
        ...
        with col1:
            st.metric("Nivel", 5)
        with col2:
            st.metric("XP Total", 1250)
        with col3:
            st.metric("Misiones Completadas", 8)
        
        st.markdown("---")
        st.markdown("### Habilidades Desbloqueadas")
        skills = ["ECG Básico", "Respiración", "SpO₂", "HRV", "Gemelo Digital", "Diagnóstico Diferencial"]
        for skill in skills:
            st.markdown(f"✅ {skill}")
...
if __name__ == '__main__' or True:
    try:
        ...
        st.exception(f"Error en Academia: {e}")


def run():
    """Wrapper entrypoint for academia_inteligente."""
